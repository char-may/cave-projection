...
# Portal will play scale in animation, and after a delay play scale out and destroy itself
# It will always rotate at the set speed

func _ready() -> void:
	# do scale in tween
	var scale_in = create_tween()
	scale_in.tween_property(portal_sprite,"scale", scale_to, scale_in_time).set_trans(Tween.TRANS_CUBIC)
	await scale_in.finished
	var scale_out = create_tween()
	scale_out.tween_property(portal_sprite,"scale", Vector2(0,0), scale_out_time).set_trans(Tween.TRANS_CIRC)
...
func _process(delta):
	...
	portal_sprite.rotation += rotation_speed * delta
